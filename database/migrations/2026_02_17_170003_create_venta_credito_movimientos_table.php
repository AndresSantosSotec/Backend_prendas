...
{
    /**
     * Tabla de movimientos de créditos de ventas (Kardex)
     * Paralela a credito_movimientos pero para ventas a crédito
     * Registra todos los pagos de cuotas de ventas a crédito
     */
    public function up(): void
    {
        Schema::create('venta_credito_movimientos', function (Blueprint $table) {
            $table->id();
            ...
            $table->foreignId('cuota_id')->nullable()->constrained('venta_credito_plan_pagos', 'id')->comment('Cuota pagada si aplica');
            ...
            $table->enum('tipo_movimiento', [
                'pago',                 // Pago normal de cuota
                'pago_parcial',         // Pago parcial de cuota
                'pago_total',           // Liquidación total del crédito
                'pago_adelantado',      // Pago adelantado de cuotas
                'enganche',             // Registro del enganche inicial
                ...
                'devolucion'            // Devolución del producto
            ])->comment('Tipo de movimiento');

            $table->integer('numero_cuota')->default(0)->comment('Número de cuota pagada (0 para otros movimientos)');
            ...
            $table->decimal('saldo_total', 20, 2)->default(0)->comment('Saldo total después del movimiento');
            ...
            $table->foreignId('movimiento_reversa_id')->nullable()->constrained('venta_credito_movimientos')->comment('Movimiento de reversa asociado');
            ...
            $table->foreignId('caja_apertura_cierre_id')->nullable()->constrained('caja_apertura_cierres')->comment('Caja donde se registró el pago');
            ...
            $table->index(['venta_credito_id', 'fecha_movimiento'], 'vcm_credito_fecha_idx');
        });
    }
    ...
    {
        Schema::dropIfExists('venta_credito_movimientos');
    }
};
