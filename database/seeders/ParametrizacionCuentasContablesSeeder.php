...
class ParametrizacionCuentasContablesSeeder extends Seeder
{
    /**
     * Seeder para insertar la parametrización estándar de cuentas contables.
     * Mapea cada tipo de operación del sistema a sus cuentas contables (debe/haber).
     *
     * Prerequisitos: deben existir las cuentas en ctb_nomenclatura y los tipos de póliza en ctb_tipo_poliza.
     * Se puede ejecutar con: php artisan db:seed --class=ParametrizacionCuentasContablesSeeder
     */
    public function run(): void
    {
        ...
        // Obtener IDs de tipos de póliza
        ...
        $cuentasRequeridas = [
            ...
            'inventario_prendas_venta', 'ventas', 'bancos'
        ];
        ...
        if (!empty($faltantes)) {
            $this->command->error('Faltan las siguientes cuentas en ctb_nomenclatura: ' . implode(', ', $faltantes));
            $this->command->info('Ejecute primero: php artisan db:seed --class=PlanCuentasSeeder');
            return;
        }

        // Verificar que existan los tipos de póliza
        if (!$polizas['ingreso'] || !$polizas['egreso'] || !$polizas['diario']) {
            $this->command->error('Faltan tipos de póliza en ctb_tipo_poliza (PI, PE, PD).');
            $this->command->info('Ejecute primero: php artisan db:seed --class=TipoPolizaSeeder');
            return;
        }
        ...
        $parametrizaciones = [
            ...
            ['credito_cancelacion', 'debe', $cuentas['caja'], $polizas['ingreso'], 'Entrada de efectivo por cancelación', 1],
            ['credito_cancelacion', 'haber', $cuentas['creditos_por_cobrar'], $polizas['ingreso'], 'Liquidación de cuenta por cobrar', 2],
            ...
            ['caja_apertura', 'haber', $cuentas['caja'], $polizas['diario'], 'Asignación desde caja general', 2],
            ...
            ['ajuste_inventario', 'debe', $cuentas['inventario_prendas_venta'], $polizas['diario'], 'Ajuste de inventario', 1],
            ['ajuste_inventario', 'haber', $cuentas['inventario_prendas_venta'], $polizas['diario'], 'Contrapartida ajuste', 2],

            // Prenda recuperada (cliente no paga, prenda pasa a inventario para venta)
            ['prenda_recuperada', 'debe', $cuentas['inventario_prendas_venta'], $polizas['diario'], 'Prenda pasa a inventario para venta', 1],
            ['prenda_recuperada', 'haber', $cuentas['creditos_por_cobrar'], $polizas['diario'], 'Cancelación de crédito incobrable', 2],

            // Prenda extraviada
            ['prenda_extraviada', 'haber', $cuentas['inventario_prendas_venta'], $polizas['diario'], 'Baja de inventario por extravío', 1],
        ];
        ...
        foreach ($parametrizaciones as $param) {
            // Verificar si ya existe esta parametrización
            $existe = DB::table('ctb_parametrizacion_cuentas')
                ->where('tipo_operacion', $param[0])
                ->where('tipo_movimiento', $param[1])
                ->where('cuenta_contable_id', $param[2])
        ...
        $this->command->info("Parametrización de cuentas contables completada:");
        $this->command->info("  - Insertadas: {$insertados}");
        if ($omitidos > 0) {
            $this->command->info("  - Omitidas (ya existían): {$omitidos}");
        }
    }
