        ...
        <table width="100%">
            <tr>
                ...
                <td width="50%" style="text-align: center; vertical-align: middle;">
                    ...
                    <h2>{{ $tituloDocumento ?? 'BALANCE GENERAL' }}</h2>
                    <div class="periodo">Al {{ \Carbon\Carbon::parse($fechaCorte)->format('d/m/Y') }}</div>
                </td>
                <td width="25%"></td>
            </tr>
        </table>
...
<div style="margin-top: 25px; font-size: 9px; text-align: justify; line-height: 1.5; color: #111;">
    EL INFRASCRITO PERITO CONTADOR {{ strtoupper($peritoContadorNombre ?? 'ROSAURA MARISOL MENDOZA YOJCOM') }} CON NÚMERO DE REGISTRO {{ $peritoContadorRegistro ?? '115855092' }} AUTORIZADO POR LA SUPERINTENDENCIA DE ADMINISTRACIÓN TRIBUTARIA (SAT), CERTIFICA: QUE EL PRESENTE BALANCE GENERAL AL {{ \Carbon\Carbon::parse($fechaCorte)->format('d/m/Y') }} DE LA ENTIDAD {{ strtoupper($empresa['nombre'] ?? 'MICROSYSTEM PLUS') }}, ASCIENDE A LA CANTIDAD DE Q. {{ number_format($totales['total_activos'], 2) }}.
</div>
